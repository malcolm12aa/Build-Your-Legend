(function installBuildYourLegendV50(){
  ...
  function hub_world_screen(){
    ...
    safeChoices([
      ['🗼 Battle Towers', openBattleTowers],
  ...
  // Battle Towers were moved to js/screens/battle-tower-screen.js in v0.59.
  // Keep this tiny bridge so old hub buttons still open the new screen without
  // keeping the old tower state machine alive.
  function openBattleTowers(){
    ensureV50State();
    G.tower_run = null;
    if (window.BYLBattleTowers && typeof window.BYLBattleTowers.screen === 'function') {
      return window.BYLBattleTowers.screen();
    }
    if (typeof window.battle_towers_screen === 'function' && window.battle_towers_screen !== openBattleTowers) {
      return window.battle_towers_screen();
    }
    clear();
    say('— BATTLE TOWERS —', 'highlight');
    say('Battle Tower screen is still loading. Return to the hub and try again.', 'narrator');
  ...
  if (old.winBattle) {
    window.winBattle = async function(){
      ...
      setTimeout(function(){
        // Tower victory routing now lives in js/screens/battle-tower-screen.js.
  ...
  if (typeof map_screen === 'function') { window.map_screen = openBattleTowers; try { map_screen = openBattleTowers; } catch(e){} }
  if (typeof raid_map_screen === 'function') { window.raid_map_screen = openBattleTowers; try { raid_map_screen = openBattleTowers; } catch(e){} }
  if (typeof show_map === 'function') { window.show_map = openBattleTowers; try { show_map = openBattleTowers; } catch(e){} }
  ...
  window.battle_towers_screen = openBattleTowers;
