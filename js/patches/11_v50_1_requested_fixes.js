...
function raceEvolutionText(r){
  var paths=(window.FANTASY_RACE_PATHS_V07&&window.FANTASY_RACE_PATHS_V07[r.name])||null;
  ...
  return paths.map(function(p){
    ...
    return "<b>"+esc(p.branch||"Evolution Path")+"</b>"+esc(names);
  }).join("<br>");
}
function raceRole(r){
  var b=(String(r.name||"")+" "+String(r.desc||"")+" "+String(r.anime||"")).toLowerCase();
  if(/dwarf|golem|stone|iron|armor|shield|titan/.test(b))return"Durable Defender";
  if(/orc|giant|dragon|oni|beast|wolf|warrior/.test(b))return"Physical Powerhouse";
  if(/elf|fairy|spirit|mage|arcane|mana|angel/.test(b))return"Mystic Caster";
  if(/dark|demon|undead|vampire|shadow|curse|abyss/.test(b))return"Dark Hybrid";
  if(/halfling|goblin|cat|fox|thief|trick|quick|wind/.test(b))return"Agile Specialist";
  if(/human|hero|adapt|balanced/.test(b))return"Flexible All-Rounder";
  return"Fantasy Origin";
}
function raceBuildNote(r,st,role){
  var t=top(st,3).map(function(k){return STAT_LABELS[k];});
  var l=low(st,2).map(function(k){return STAT_LABELS[k];});
  return esc(r.name)+" is strongest as a "+role.toLowerCase()+" foundation. Prioritize "+t[0]+", "+t[1]+", and "+t[2]+" when choosing your first job, then use later class paths to cover "+l[0]+" or "+l[1]+". Max this base race before chasing its evolution paths so the race capstone stays part of your long-term build.";
}
function raceCard(id,r){
  var st=profile(r,600,"race-"+id,"race"),t=top(st,2),l=low(st,2),role=raceRole(r);
  return '<div class="v501-card-title">'+esc(r.name)+'</div>'+
  '<div class="v501-card-meta">Base Race · '+esc(r.anime||"Fantasy")+' · Max Level: '+esc(r.max_lv||"?")+' · Role: '+esc(role)+' · BST 600</div>'+
  '<div class="v501-field"><b>Status Layout</b>'+statHTML(st,600)+'</div>'+
  '<div class="v501-desc"><b>Unique Description:</b> '+esc(r.desc||"")+'</div>'+
  '<div class="v501-grid">'+
    '<div class="v501-field"><b>Evolution Paths</b>'+raceEvolutionText(r)+'</div>'+
    '<div class="v501-field"><b>Core Identity</b>'+esc(role)+' origin with '+STAT_LABELS[t[0]]+' and '+STAT_LABELS[t[1]]+' as its natural lead stats.</div>'+
    '<div class="v501-field"><b>Strengths</b>Excels at '+STAT_LABELS[t[0]]+' and '+STAT_LABELS[t[1]]+'.</div>'+
    '<div class="v501-field"><b>Weaknesses</b>Lower '+STAT_LABELS[l[0]]+' and '+STAT_LABELS[l[1]]+'.</div>'+
  '</div>'+
  '<div class="v501-desc"><b>Unique Build Note:</b> '+raceBuildNote(r,st,role)+'</div>';
}
...
function rerenderOldRaceSelectionV51(){
  setTimeout(function(){
    try{
      var o=out(), c=ch();
      if(!o||!c||hasGame())return;
      var text=((o.textContent||"")+" "+(c.textContent||""));
      var oldRace=/YGGDRASIL\s*[—-]\s*FANTASY RACE REDO|STEP\s*1:\s*CHOOSE YOUR BASE RACE|COMMON FANTASY RACES/i.test(text);
      var alreadyNew=!!c.querySelector(".v501-select")||/RACE SELECTION\s*[—-]\s*BASE PROFILE/i.test(text);
      if(oldRace&&!alreadyNew)pickRace();
    }catch(e){}
  },0);
}
rerenderOldRaceSelectionV51();
})();
